...
TAC* codeGenerator(AST* node)
{
	...
	switch(node->type)
	{
		...
		case	AST_ATRVEC: result = makeAtrVec(node->symbol,code[0],code[1]);
			break;			
		case	AST_DECINIT: {result = tacJoin(code[0], tacCreate(TAC_DECINIT, node->symbol, node->son[1]->symbol,0)); /*fprintf(stderr, "declaracao --> code0 %d   code1 %d   code2 %d   code3 %d   : %d\n", code[0],code[1],code[2],code[3]);*/}
			break;			
